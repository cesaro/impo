...
The OPTIONS placeholder corresponds to zero or more of the following options:
 ...
 --k0=CONSTRAINT
   Adds CONSTRAINT to the final constraint generated by the tool.
   Examples:
    - 'x <=y'
    - 'x > y & z == 7'
    - 'x == y or z > 3 & x < 20' ('&' has higher precedence than 'or')

 --output=PATH
   Save the generated constraint to the file PATH
 ...
 --max-events=N
   ...
   Note that employing an incomplete unfolding may lead to the generation of an
   unsound parameter constraint (some valuations included in the resulting
   constraint may induce TPNs whose timed unfolding is larger than the timed
   unfolding of the reference valuation).
...
class Main :
    ...
    def parse_cmdline_args (self) :
        ...
        p.add_argument ("--seq-semantics", action="store_true")
        ...
        if args.max_events != None :
            self.arg_max_events = args.max_events
        self.arg_no_asserts = args.no_asserts
        self.arg_no_unlink = args.no_unlink
        self.arg_seq_sem = args.seq_semantics
        self.arg_k0 = args.k0
    ...
    def main (self) :
        ...
        # if we have been requested for sequential semantics, transform the PES
        # into a computation tree (a PES with no concurrency)
        if self.arg_seq_sem :
            print 'impo: pes > ct: converting the PES into its computation tree (--seq-sem)'
            self.pes = pes.pes_to_ct (self.pes)
            print 'impo: pes > ct: done, %d events' % len (self.pes.events)
        ...
        # save results
        if self.arg_output_path != None :
            print "impo: saving constraint to '%s'" % self.arg_output_path
            f = open (self.arg_output_path, 'w')
            f.write (self.final_const + '\n')
            f.close ()
    ...
    def export_and_unfold (self) :
        ...
        try :
            ...
            if self.arg_max_events != None :
                cmd.append ('--max-events=%d' % self.arg_max_events)
            ...
            print 'impo: net > unf: stdout:', long_str (repr (out), 120)
